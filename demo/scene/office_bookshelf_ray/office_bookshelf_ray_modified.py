...
for i in range(num_envs):
    mujoco_envs[i].init_set_state.remote(scene='bookshelf_modified')
...
positions = ray.get(mujoco_envs[0].get_sampled_position.remote(offset=[0,0,0.0], scene='bookshelf_modified', threshold=0.05, 
                    resolution_table=(9,9,9), resolution_shelf=(6,6,6), resolution_obj=(15,15,1),
                    add_table=True, x_range=[0.40, 1.15], z_range=[0.7, 1.3]))
...
np.save('./data/modified/filtered_pcd.npy', positions)
...
for env_idx, env in remaining_envs:
    env.get_feasible_position.remote(splited_positions[env_idx], scene='office_bookshelf', var_threshold=150, VERBOSE=True, 
                                     quat_bounds = [0.92, 1.00], perturb_tick = 500, noise_scale = 0.005,
...
if os.path.isfile(f'./data/modified/feasible_pcd_{file_name}_c{num_cpus}_g{num_gpus}_worker{num_envs}.npy'):
    np.save(f'./data/modified/feasible_pcd_{file_name}_c{num_cpus}_g{num_gpus}_worker{num_envs}_2.npy', feasible_points)
    print(f"[Renamed] The file was successfully saved!: {f'./data/modified/feasible_pcd_{file_name}_c{num_cpus}_g{num_gpus}_worker{num_envs}_2.npy'}")
else:
    np.save(f'./data/modified/feasible_pcd_{file_name}_c{num_cpus}_g{num_gpus}_worker{num_envs}.npy', feasible_points)
    print(f"The file was successfully saved!: {f'./data/modified/feasible_pcd_{file_name}_c{num_cpus}_g{num_gpus}_worker{num_envs}.npy'}")

if os.path.isfile(f'./data/modified/quat_range_{file_name}_c{num_cpus}_g{num_gpus}_worker{num_envs}.npy'):
    np.save(f'./data/modified/quat_range_{file_name}_c{num_cpus}_g{num_gpus}_worker{num_envs}_2.npy', quat_ranges)
    print(f"[Renamed] The file was successfully saved!: {f'./data/modified/quat_range_{file_name}_c{num_cpus}_g{num_gpus}_worker{num_envs}_2.npy'}")
else:
    np.save(f'./data/modified/quat_range_{file_name}_c{num_cpus}_g{num_gpus}_worker{num_envs}.npy', quat_ranges)
    print(f"The file was successfully saved!: {f'./data/modified/quat_range_{file_name}_c{num_cpus}_g{num_gpus}_worker{num_envs}.npy'}")
