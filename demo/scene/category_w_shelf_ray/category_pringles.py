...
config['num_cpus'] = 48
config['num_gpus'] = 4
config['file_name'] = "20230915" # config['file_name']
...
for i in range(num_envs):
    mujoco_envs[i].init_set_state.remote(scene='category_pringles')
...
positions = ray.get(mujoco_envs[0].get_sampled_position.remote(offset=[0,0,0.02], scene='category_pringles', threshold=0.05, 
                    resolution_table=(9,9,9), resolution_shelf=(6,6,6), resolution_obj=(15,15,1),
                    add_table=False, x_range=[0.000, 1.15], z_range=[0.7, 1.3]))
...
np.save('./data/filtered_pcd.npy', positions)
...
for env_idx, env in remaining_envs:
    env.get_feasible_position.remote(splited_positions[env_idx], scene='category_pringles', var_threshold=150, VERBOSE=True, 
                                     quat_bounds = [0.696, 0.72], perturb_tick = 500, noise_scale = 0.003,
                                     target_obj_name=target_object_name, offset=np.array([0, 0, 0.07]), init_q=init_ur_q, nstep=100, end_tick=3000)
...
if os.path.isfile(f'./data/feasible_pcd_{file_name}_c{num_cpus}_g{num_gpus}_worker{num_envs}.npy'):
    np.save(f'./data/feasible_pcd_{file_name}_c{num_cpus}_g{num_gpus}_worker{num_envs}_2.npy', feasible_points)
    print(f"[Renamed] The file was successfully saved!: {f'./data/feasible_pcd_{file_name}_c{num_cpus}_g{num_gpus}_worker{num_envs}_2.npy'}")
else:
    np.save(f'./data/feasible_pcd_{file_name}_c{num_cpus}_g{num_gpus}_worker{num_envs}.npy', feasible_points)
    print(f"The file was successfully saved!: {f'./data/feasible_pcd_{file_name}_c{num_cpus}_g{num_gpus}_worker{num_envs}.npy'}")

if os.path.isfile(f'./data/quat_range_{file_name}_c{num_cpus}_g{num_gpus}_worker{num_envs}.npy'):
    np.save(f'./data/quat_range_{file_name}_c{num_cpus}_g{num_gpus}_worker{num_envs}_2.npy', quat_ranges)
    print(f"[Renamed] The file was successfully saved!: {f'./data/quat_range_{file_name}_c{num_cpus}_g{num_gpus}_worker{num_envs}_2.npy'}")
else:
    np.save(f'./data/quat_range_{file_name}_c{num_cpus}_g{num_gpus}_worker{num_envs}.npy', quat_ranges)
    print(f"The file was successfully saved!: {f'./data/quat_range_{file_name}_c{num_cpus}_g{num_gpus}_worker{num_envs}.npy'}")
