...
from utils.util import fill_object, get_geom_region_type5, passthrough_filter, remove_duplicates_with_threshold, \
                        r2quat, rpy2r
...
config['num_cpus'] = 48
config['num_gpus'] = 4
config['file_name'] = "exp_0914_" # config['file_name']
...
for i in range(num_envs):
    mujoco_envs[i].init_set_state.remote(scene='kitchen_small')
...
positions = ray.get(mujoco_envs[0].get_sampled_position.remote(offset=[0,0,0.12], scene='kitchen_small', threshold=0.01, 
                    resolution_table=(9,9,9), resolution_shelf=(6,6,6), resolution_obj=(20,20,1),
                    add_table=False, x_range=[0.40, 1.15], z_range=[0.7, 1.3]))
...
np.save(f'./data/small/filtered_pcd_{file_name}.npy', positions)
...
for env_idx, env in remaining_envs:
    env.get_feasible_position.remote(splited_positions[env_idx], scene='kitchen_small', var_threshold=150, VERBOSE=True, 
                                    quat_bounds = [0.45, 0.56], perturb_tick = 500, noise_scale = 0.0015,
                                    target_obj_name=target_object_name, offset=np.array([0, 0, 0.05]), init_q=init_ur_q, nstep=100, end_tick=3000)
...
if os.path.isfile(f'./data/small/feasible_pcd_{file_name}_c{num_cpus}_g{num_gpus}_worker{num_envs}.npy'):
    np.save(f'./data/small/feasible_pcd_{file_name}_c{num_cpus}_g{num_gpus}_worker{num_envs}_2.npy', feasible_points)
    print(f"[Renamed] The file was successfully saved!: {f'./data/small/feasible_pcd_{file_name}_c{num_cpus}_g{num_gpus}_worker{num_envs}_2.npy'}")
else:
    np.save(f'./data/small/feasible_pcd_{file_name}_c{num_cpus}_g{num_gpus}_worker{num_envs}.npy', feasible_points)
    print(f"The file was successfully saved!: {f'./data/small/feasible_pcd_{file_name}_c{num_cpus}_g{num_gpus}_worker{num_envs}.npy'}")

if os.path.isfile(f'./data/small/quat_range_{file_name}_c{num_cpus}_g{num_gpus}_worker{num_envs}.npy'):
    np.save(f'./data/small/quat_range_{file_name}_c{num_cpus}_g{num_gpus}_worker{num_envs}_2.npy', quat_ranges)
    print(f"[Renamed] The file was successfully saved!: {f'./data/small/quat_range_{file_name}_c{num_cpus}_g{num_gpus}_worker{num_envs}_2.npy'}")
else:
    np.save(f'./data/small/quat_range_{file_name}_c{num_cpus}_g{num_gpus}_worker{num_envs}.npy', quat_ranges)
    print(f"The file was successfully saved!: {f'./data/small/quat_range_{file_name}_c{num_cpus}_g{num_gpus}_worker{num_envs}.npy'}")
