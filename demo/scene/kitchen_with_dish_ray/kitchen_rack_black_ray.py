...
from utils.util import passthrough_filter, remove_duplicates_with_threshold, get_geom_region_type6, r2quat, rpy2r
import ray
import time
import os
import sys
...
for i in range(num_envs):
    mujoco_envs[i].init_set_state.remote(scene='kitchen_black')
...
positions = ray.get(mujoco_envs[0].get_sampled_position.remote(offset=[0,0,0.12], scene='kitchen_black', threshold=0.02, 
...
np.save(f'./data/black/filtered_pcd_{file_name}.npy', positions)
...
# for env_idx, env in tqdm(remaining_envs, desc="Processing environments", total=len(remaining_envs)):
    env.get_feasible_position.remote(splited_positions[env_idx], scene='kitchen_black', var_threshold=150, VERBOSE=True, 
                                     quat_bounds = [0.3, 0.60], perturb_tick = 500, noise_scale = 0.0015,
                                     target_obj_name=target_object_name, offset=np.array([0, 0, 0.05]), init_q=init_ur_q, nstep=100, end_tick=1500)
...
if os.path.isfile(f'./data/black/feasible_pcd_{file_name}_c{num_cpus}_g{num_gpus}_worker{num_envs}.npy'):
    np.save(f'./data/black/feasible_pcd_{file_name}_c{num_cpus}_g{num_gpus}_worker{num_envs}_2.npy', feasible_points)
    print(f"[Renamed] The file was successfully saved!: {f'./data/black/feasible_pcd_{file_name}_c{num_cpus}_g{num_gpus}_worker{num_envs}_2.npy'}")
else:
    np.save(f'./data/black/feasible_pcd_{file_name}_c{num_cpus}_g{num_gpus}_worker{num_envs}.npy', feasible_points)
    print(f"The file was successfully saved!: {f'./data/black/feasible_pcd_{file_name}_c{num_cpus}_g{num_gpus}_worker{num_envs}.npy'}")

if os.path.isfile(f'./data/black/quat_range_{file_name}_c{num_cpus}_g{num_gpus}_worker{num_envs}.npy'):
    np.save(f'./data/black/quat_range_{file_name}_c{num_cpus}_g{num_gpus}_worker{num_envs}_2.npy', quat_ranges)
    print(f"[Renamed] The file was successfully saved!: {f'./data/black/quat_range_{file_name}_c{num_cpus}_g{num_gpus}_worker{num_envs}_2.npy'}")
else:
    np.save(f'./data/black/quat_range_{file_name}_c{num_cpus}_g{num_gpus}_worker{num_envs}.npy', quat_ranges)
    print(f"The file was successfully saved!: {f'./data/black/quat_range_{file_name}_c{num_cpus}_g{num_gpus}_worker{num_envs}.npy'}")
